...
export default function MeetingForm() {
  ...
  return (
    <form className="bg-secondary p-6 rounded-lg w-full max-w-[500px]" onSubmit={onHandleSubmit}>
      ...
      <select name="meeting-type" id="meeting-type" className="w-full px-4 py-2 rounded-lg text-black mb-4" onChange={onTypeChange}>
      ...
      <div className={showDateTime ? 'block' : 'hidden'}>
        ...
        <input type="date" name="meeting-date" id="meeting-date" className="w-full px-4 py-2 rounded-lg text-black mb-4" />

        <label htmlFor="meeting-time" className="font-medium block mb-2">Meeting Time</label>
        <input type="time" name="meeting-time" id="meeting-time" className="w-full px-4 py-2 rounded-lg text-black mb-4" />
      </div>
